...
describe("voting-dapp", () => {
    ...
    function getNewProgramInteraction(): { user: anchor.web3.Keypair, program: Program<VotingDapp>, provider: anchor.Provider } {
        ...
        const provider = new anchor.AnchorProvider(anchor.AnchorProvider.env().connection, new anchor.Wallet(user), {});
    ...
    it("Base user: Initialize OK - counter with 0 votes for TeamA and teamb", async () => {
    ...
    it("New user: Vote ERROR - no prior credit for paying transaction fee", async () => {
        ...
        assert.equal(votesCounterAccount.teamB.toString(), '0')
        try {
            const tx = await program.methods.vote({teamB: {}}).accounts({
                user: user.publicKey,
                systemProgram: SystemProgram.programId,
    ...
    it("New user: Vote ERROR - insufficient funds for paying voting service fee", async () => {
        ...
        assert.equal(votesCounterAccount.teamB.toString(), '0')
        try {
            const tx = await program.methods.vote({team_b: {}}).accounts({
                user: user.publicKey,
                systemProgram: SystemProgram.programId,
                votesCounter: votesCounterPDA
            }).rpc();
            ...
            expect(err).to.be.instanceOf(SendTransactionError)
            assert.ok(err.logs.some((log: string | string[]) =>
    ...
    it("New user: Vote OK - votes for TeamA and pays voting service fee", async () => {
        const {user, program, provider} = getNewProgramInteraction();
        ...
        assert.equal(votesCounterAccount.teamA.toString(), '0')
        assert.equal(votesCounterAccount.teamB.toString(), '1')
        const tx = program.methods.vote({teamA: {}}).accounts({
            user: user.publicKey,
            systemProgram: SystemProgram.programId,
            votesCounter: votesCounterPDA
        })
        ...
        assert.deepEqual(userVoteAccount.vote, {teamA: {}})
    ...
    it("Base user: Vote ERROR - Fails to vote again", async () => {
        ...
        try {
            const tx = await base_program.methods.vote({teamB: {}}).accounts({
                user: base_user.publicKey,
                votesCounter: votesCounterPDA,
                systemProgram: SystemProgram.programId,
            }).rpc();
